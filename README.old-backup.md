...
**One prompt. Two AI employees. One reviewed, runnable product.**
...
ClaudeX can stop at `blueprint.md`, or continue into a controlled implementation
workspace. It turns the accepted blueprint into dependency-ordered tasks, makes
Claude and GPT alternate implementation and review, validates every proposed
path, writes the reviewed files, runs a human-approved verification command,
and resumes from the last accepted task after interruption.
...
```bash
python -m claudex create "A browser space shooter with enemy waves and boss battles" --via cli --verify-command "npm test"
```

This performs planning, adversarial review, blueprint assembly, implementation,
cross-vendor code review, and verification. Generated files are placed in
`runs/<project>/product/`. Model-suggested commands are never executed; only the
command explicitly supplied with `--verify-command` may run, without a shell.
...
| `create "<idea>"` | one prompt → blueprint → reviewed product |
| `run` | the debate |
| `build` | re-assemble outputs without re-debating |
| `make` | build or resume a product from an accepted blueprint |
...
├── product/               ← generated runnable product files
